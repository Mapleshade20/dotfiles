---
name: design
description: Invoke by Designer when building any UI, component, page, or visual interface. Produces distinctive design with a committed aesthetic, not generic defaults. Handles screenshot-driven iteration when the user sends an image with a complaint. Not for backend logic or data pipelines.
metadata:
  version: "3.12.0"
---
...
`references/design-reference.md` is already loaded during direction lock; if it's not, find it at `~/.config/opencode/skills/design/references/design-reference.md`. It owns the full rules: typography, OKLCH color, motion timings, layout defaults, CSS-pattern bans, accessibility baseline, and complexity matching. Apply them. Do not restate them here.
